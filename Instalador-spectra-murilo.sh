...
ls | grep "jar-individual-murilo.jar" &>/dev/null
...
if [ $? = 0 ];
    then 
        ...
        sudo docker start spectra_container
        java -jar jar-individual-murilo.jar 
    else 
        echo "Vamos instalar a aplicação Spectra!"
sleep 5
wget https://github.com/Spectra-Solutions/Spectra-Java/releases/download/v1.0Mu/jar-individual-murilo.jar &>/dev/null

sudo docker start spectra_container

sudo chmod +x jar-individual-murilo.jar
java -jar jar-individual-murilo.jar
fi 