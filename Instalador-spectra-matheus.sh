...
ls | grep "jar-individual-matheus.jar" &>/dev/null
...
if [ $? = 0 ];
    then 
        ...
        sudo docker start spectra_container
        java -jar jar-individual-matheus.jar 
    else 
        echo "Vamos instalar a aplicação Spectra!"
sleep 5
wget https://github.com/Spectra-Solutions/Spectra-Java/releases/download/v1.0M/jar-individual-matheus.jar &>/dev/null

sudo docker start spectra_container

sudo chmod +x jar-individual-matheus.jar
java -jar jar-individual-matheus.jar
fi 